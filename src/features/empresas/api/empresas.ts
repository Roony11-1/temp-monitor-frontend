...
export async function getEmpresasPage(page: number, size: number, filters?: Record<string, string>) {
  ...
  const res = await api.get<PaginatedResponse<Empresa>>(ApiConfig.empresas.list, { params })
  return res.data
}
